...
type SceneId = 'capture' | 'summary' | 'ask' | 'team' | 'cta';
...
const SCENES: Scene[] = [
  { id: 'capture', durationMs: 6500 },
  { id: 'summary', durationMs: 5500 },
  { id: 'ask',     durationMs: 6500 },
  { id: 'team',    durationMs: 5500 },
...
export function DemoReel({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={`relative w-full ${compact ? 'max-w-3xl' : 'max-w-5xl'} mx-auto`}>
      ...
      <div className="relative rounded-2xl border border-emerald-900/30 bg-[#0a0f0d] shadow-2xl overflow-hidden">
        {/* Mock window bar */}
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-emerald-900/30 bg-[#080c0a]/60">
          <span className="w-2.5 h-2.5 rounded-full bg-red-400/70" />
          <span className="w-2.5 h-2.5 rounded-full bg-amber-400/70" />
          <span className="w-2.5 h-2.5 rounded-full bg-emerald-400/70" />
          <div className="ml-3 flex-1 text-center">
            <span className="text-[11px] text-gray-500 font-mono">briva.app/meetings/q3-roadmap</span>
          </div>
          <SceneTimeline current={sceneIndex} />
        </div>

        {/* Scene viewport — fixed aspect for stable embedding */}
        <div className={`relative ${compact ? 'h-[420px]' : 'h-[520px] md:h-[560px]'} bg-[#080c0a] overflow-hidden`}>
          {/* Brand glow backdrop */}
...
function sceneCaption(s: SceneId): string {
  switch (s) {
    case 'capture': return 'Live capture — speakers, languages, and timestamps detected automatically.';
    case 'summary': return 'AI summary with structured action items, the moment recording stops.';
    case 'ask':     return 'Ask BRIVA AI anything about the meeting — or any meeting your team has run.';
    ...
    <div className="absolute inset-0 p-6 md:p-8 flex flex-col">
      <div className="flex items-center justify-between mb-5 animate-fade-in">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 rounded-full bg-purple-500/15 text-purple-200 text-[11px] font-medium border border-purple-500/30">
            粵語
          </span>
        </div>
      </div>
